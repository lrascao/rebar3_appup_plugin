...
merge_instructions([AppUpFile], UpgradeInstructions, DowngradeInstructions,
    ...
    rebar_api:debug(".appup.pre.src contents:\n~p\n"
                    ".appup.post.src contents:\n~p\n",
                    [PreContents, PostContents]),
...
matching_versions(Pattern, Version) ->
    ...
    rebar_api:debug("Checking if pattern '~s' matches version '~s'\n"
                    "Result = ~p\n",
                    [Pattern, Version, Res]),
    Res.
